...
export default function ChatLateral() {
  ...
  // Conectar ao WebSocket quando o chat Ã© aberto
  useEffect(() => {
    ...
    return () => {
      if (socketRef.current) {
        ...
        socketRef.current = null; // Limpar referÃªncia para permitir reconexÃ£o
      }
    };
  }, [aberto]);
  ...
  const conectarWebSocket = () => {
    ...
    try {
      ...
      socket.onopen = () => {
        ...
        adicionarMensagemSistema('OlÃ¡! Bem-vindo ao atendimento Saluvia. Como posso ajudÃ¡-lo?');
      };
      ...
      socket.onerror = (erro) => {
        ...
        socketRef.current = null; // Limpar referÃªncia em caso de erro
        setConectado(false);
      };

      socket.onclose = () => {
        setConectado(false);
        socketRef.current = null; // Limpar referÃªncia ao fechar
        console.log('WebSocket desconectado');
        
        // Tentar reconectar apÃ³s 3 segundos se o chat ainda estiver aberto
        setTimeout(() => {
          if (aberto && !socketRef.current) {
  ...
  const enviarMensagem = () => {
    ...
    // Se Ã© a primeira mensagem, pedir o nome
    if (!nomeCliente) {
      setNomeCliente(mensagemAtual.trim());
      adicionarMensagemSistema(`Prazer, ${mensagemAtual.trim()}! Como posso ajudar vocÃª hoje?`);
      setMensagemAtual('');
      return;
    }
    ...
    // Simular resposta automÃ¡tica (remover quando tiver backend real)
    setTimeout(() => {
      adicionarMensagemSistema('Obrigado pela mensagem! Um de nossos consultores responderÃ¡ em breve.');
    }, 1500);
  };
    ...
    <div className="fixed bottom-6 right-6 z-40 w-[380px] max-w-[calc(100vw-3rem)]">
      <Card className="shadow-2xl border-none overflow-hidden">
        ...
        <CardHeader className="bg-saluviaGreen text-white p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="flex items-center justify-center w-10 h-10 rounded-full bg-saluviaGreen text-saluviaBeige/20">
                <MessageSquare className="w-5 h-5" />
              </div>
              <div>
                ...
                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${conectado ? 'bg-green-400' : 'bg-saluviaGreen text-saluviaBeige'}`}></div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="p-1.5 rounded hover:bg-saluviaGreen text-saluviaBeige/10 transition-colors"
        ...
        <CardContent className="p-0">
          ...
          >
                ...
                <div
                  className={`max-w-[80%] rounded-lg px-4 py-2 ${
                    msg.tipo === 'cliente'
                      ...
                      : 'bg-saluviaGreen text-saluviaBeige  shadow-sm'
                  }`}
                >
            ...
            {digitando && (
              <div className="flex justify-start">
                <div className="bg-saluviaGreen text-saluviaBeige rounded-lg px-4 py-2 shadow-sm">
                  <div className="flex gap-1">
                    <div className="w-2 h-2 rounded-full bg-saluviaGreen text-saluviaBeige animate-bounce"></div>
                    <div className="w-2 h-2 rounded-full bg-saluviaGreen text-saluviaBeige animate-bounce delay-100"></div>
                    <div className="w-2 h-2 rounded-full bg-saluviaGreen text-saluviaBeige animate-bounce delay-200"></div>
                  </div>
                </div>
              </div>
            )}
          ...
          {/* Ãrea de input */}
          <div className="p-4 bg-saluviaGreen text-saluviaBeige border-t border-gray-200">
            <form
              onSubmit={(e) => {
                e.preventDefault();
                enviarMensagem();
