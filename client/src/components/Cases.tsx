...
const depoimentos = [
  ...
  {
    texto: "Parceria confiável, produtos certificados e atendimento personalizado. Exatamente o que precisávamos.",
    autor: "Dra. Ana Paula",
...
export default function Cases() {
  return (
    <section className="py-16 sm:py-20 bg-saluviaBeige/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Cabeçalho */}
        <div className="text-center max-w-3xl mx-auto mb-12">
          <h2 className="text-3xl sm:text-4xl font-title font-bold text-saluviaGreen mb-4" data-testid="text-cases-titulo">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {depoimentos.map((depoimento, indice) => (
            <Card
              key={indice}
              className="bg-white border-saluviaGold/20 hover-elevate transition-all duration-300"
              data-testid={`card-depoimento-${indice}`}
            >
              <CardContent className="p-6 space-y-4">
                <Quote className="w-8 h-8 text-saluviaGold" />
                <p className="text-sm font-body text-gray-700 leading-relaxed italic">
                  "{depoimento.texto}"
                </p>
        ...
        <div className="mt-12">
          <p className="text-center text-sm font-body text-gray-500 mb-6">
            Confiado por hospitais, clínicas e laboratórios em todo o Brasil
          </p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 items-center justify-items-center opacity-60">
            {[1, 2, 3, 4].map((num) => (
              <div
                key={num}
                className="w-32 h-16 bg-saluviaBeige rounded-lg flex items-center justify-center"
